...
system: High Alert future-memory seed rule
version: 2.0.0
branch: main
---

# Seeds from High Alert

The old seed table in this path is superseded by the current **17 memory anchors for Episodes 02–18**.
...
Each later episode should make its corresponding fragment more legible, but the fragment does not determine episode ownership.
...
```text
S1E1          -> contextless future fragments
Episodes 02–18 -> human event becomes understandable
RAID           -> future chronology catches up
Evil Twin      -> some fragments gain cross-board provenance meaning
Ghost          -> PRIM2's institutional-gravity footprint becomes legible
```
