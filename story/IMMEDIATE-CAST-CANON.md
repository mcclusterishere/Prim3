---
status: CANON
system: PRIM3 immediate-story cast authority
version: 3.2.0
branch: main
---

# PRIM3 Immediate Cast Canon

## Authority and scope

This file governs the people who recur in the immediate age-33-to-40 PRIM3 story.

Root `CANON.md` remains highest authority. Character-specific psychological dossiers override short summaries here where they are more specific and non-conflicting.

Core cast law:

> **PRIM3 is a recurring workplace/community ensemble, not an anthology that invents new support people whenever an episode needs texture.**

The same people work in the same institutional ecosystem for years. They repeatedly cross paths in hallways, classes, briefings, meals, shifts, command rooms, missions, training, debriefs, arguments, parties, messages and crises.

Nationality is not a costume or substitute for characterization.

---

# 1. McCluster — canonical Wildcard

McCluster is the PRIM-side **Wildcard / Flex**.

He crosses operational seams because he repeatedly takes unowned work, learns beside specialists and combines R/E/T/Defense/logistics reasoning when the original division of labor fails.

Wildcard does **not** mean:
- magical omniskill;
- automatic authorization;
- being better than every specialist;
- owning every episode;
- being a default selectable unit in every mission.

Ordinary PLAY withholds McCluster from the normal selectable roster.

McCluster enters only when:
- a specifically authored mission scripts him from launch;
- an eligible CRITICAL mission receives a limited Wildcard call-up;
- or the story explicitly changes his availability.

> **If a task falls between chairs, McCluster picks it up.**

Character risk:

> **Being able to cover everybody's job can turn into believing he is entitled to make everybody's decisions.**

---

# 2. Canonical Equity Uprise / McCluster Corp mains — 24

1. **McCluster — United States — Wildcard / Cohort path**
2. **Aya Shinoda — Japan — Field-T Technical Operator**
3. **Arjun Mehta — India — Field-E Entry Operator**
4. **Li Wei — China — Authority / Director track**
5. **Sari Wulandari — Indonesia — Cohort-01 Operator**
6. **Hassan Raza — Pakistan — Defense-03 Tech-Zone Guard**
7. **Chioma Okeke — Nigeria — Cohort-03 Lead**
8. **Rafael Costa — Brazil — Field-R Overwatch**
9. **Nusrat Rahman — Bangladesh — Defense-02 Access Officer**
10. **Dmitri Volkov — Russia — Defense-01 Building Guard**
11. **Oksana Bondarenko — Ukraine — Field-R / Intelligence**
12. **Yonas Bekele — Ethiopia — Cohort-01 Operator**
13. **Valeria Soto — Mexico — Cohort-02 Trainee**
14. **Amira Hassan — Egypt — Authority-03 Mission Sponsor**
15. **Miguel Santos — Philippines — Defense-02 Access Officer**
16. **Grace Mbala — Democratic Republic of the Congo — Cohort Support**
17. **Minh Tran — Vietnam — Field-T Technical Operator**
18. **Leila Hosseini — Iran — Authority / Analysis**
19. **Emre Yıldız — Turkey — Defense-01 Building Guard**
20. **Jonas Weber — Germany — System Liaison / Pylon Interface**
21. **Priya Srisawat — Thailand — Marketing / Public Surface**
22. **Helen Clarke — United Kingdom — Authority-02 Director**
23. **Marc Dubois — France — Mission Sponsor**
24. **Thandiwe Nkosi — South Africa — Cohort-03 Lead peer**

These people are not McCluster's support roster. **They are the show.**

Owning one episode does not cause somebody to disappear in the next. They remain coworkers with schedules, friendships, grudges, unfinished tasks and consequences.

---

# 3. Public/system recurring characters — 3

25. **Maren Quill** — Equity Uprise communications/public-facing human surface.
26. **Pylon** — System-01 / building automation platform; recurring character-level system presence through Jonas and the physical environment.
27. **PRIM2** — presiding hidden Mantle holder during almost all of the series.

These belong to the immediate recurring environment even when remote, institutional or hidden.

---

# 4. Contemporary Hitman recurring cast — 12

28. **Royce** — contemporary Hitman successor/sovereign; McCluster's old rival; seven years younger; receives the succession opportunity McCluster refused.
29. **Sable Quinn** — operations / money.
30. **Grant Voss** — sponsor / respectable-money surface.
31. **Tone Rivera** — block/control operator.
32. **Kira “Keys” Moon** — access/social gate.
33. **Omar “Rack” Dillon** — logistics/equipment.
34. **Jules “Kiosk” Mendez** — deniable technical support.
35. **Rae “Twin” Colton** — social-entry specialist.
36. **Chris “Perch” Alvarez** — overwatch/context.
37. **Dez “Switch” Calhoun** — adaptive younger operator.
38. **Mina “Halo” Brooks** — cell lead.
39. **Sora “Gymnast” Okada** — elite/social-entry specialist.

**Benji is not a separate contemporary character. Benji is McCluster's age-19-to-26 Hitman identity.**

Royce is the primary Hitman-side elite incursion threat in PRIM3 gameplay. He is not a PRIM Mantle holder and does not possess PRIM2's information architecture.

---

# 5. Immediate recurring roster total

```text
24 Equity Uprise / McCluster Corp mains
+ 3 public/system/apex presences
+ 12 contemporary Hitman figures
= 39 recurring immediate-story slots
```

Production rule:

> **Use the 39 before inventing anybody else.**

The active count may later fall below 39 through death, transfer, disability, resignation or other story consequences. The number 39 describes the opening recurring architecture, not a promise that all 39 remain active forever.

---

# 6. Historical / outside characters

`story/CAST-NATIONS.md` contains a large Legacy/Freelancer development pool. Those names are **not automatically immediate-story canon**.

They remain:

```text
DEVELOPMENT / HISTORY POOL
unless separately promoted by specialist canon
```

Already-promoted examples include:
- Hana Lee;
- Ren Shinoda;
- Tariq Al-Farsi;
- Zayd Al-Harthi;
- Masato / Keiko Shinoda where established;
- Bigga Lex the Jet.

Historical/outside people appear because a specific relationship requires them, not because an episode needs more bodies.

Prefer recurring outside relationships over disposable exposition guests.

---

# 7. Recurrence law

The same characters repeatedly overlap through:

### Workplace recurrence
briefings, labs, halls, classes, food, transport, shifts, recovery, parties, paperwork, equipment handoff and post-mission cleanup.

### Relationship recurrence
established friendships, rivalries, flirtations, debts, mentorships and resentments keep moving even when neither person owns the episode.

### Consequence recurrence
policies, injuries, staffing gaps, missing people, workload, equipment, reputation and grief visibly carry forward.

Examples:
- Nusrat's access decision becomes somebody else's later process;
- Sari carries a backup habit after `Got Wifi`;
- Pylon's behavior changes because Jonas changed policy earlier;
- a Royce incursion leaves fear/injury/procedure changes;
- after a death, somebody covers the shift and starts becoming more important.

---

# 8. Episode-density / micro-development law

Every ordinary episode should normally contain:

> **at least 6–7 meaningful named presences, with 7–10 preferred**

when story scale permits.

Default source order:

```text
1–2 owners
3–6 additional recurring people from the immediate roster
0–2 recurring opposing-board people when relevant
0–2 established historical/outside people when specifically earned
```

Do not invent guests to hit the count.

A recurring character counts only if the episode changes or reveals something persistent about them.

Canonical test:

> **If a named recurring character appears, what is newly true about them when the episode ends?**

Valid micro-writes include relationship movement, changed habit, competence, status, workload, secrets, moral-line movement, material-life pressure, grief and future seeds.

Silent bodies do not count.

Full population authority: `story/EPISODE-CAST-DENSITY-SUPPORT-HISTORY-AUTHORITY.md`.

---

# 9. Hitman recurrence law

Hitman is also a recurring organization, not a new enemy roster every week.

Sable, Keys, Kiosk, Perch, Mina, Dez, Rack, Tone, Rae, Sora, Grant and Royce recur in combinations appropriate to their jobs and relationships.

Royce stays comparatively scarce so his arrival changes the mission, while the organization around him becomes familiar enough to care about.

---

# 10. Cast hierarchy is dynamic

Tier labels describe current narrative prominence, not permanent contracts.

A support character can move:

```text
BACKGROUND RECURRING
-> SUPPORT
-> RELIABLE RECURRING
-> SECONDARY MAIN
-> EPISODE OWNER / MAIN
```

A main can move outward through transfer, loss of clearance, injury, burnout, political drift, resignation, antagonism or death.

Do not preserve prominence because an early planning file once called somebody `main`.

---

# 11. Attrition / vacancy / replacement law

Missions are dangerous. Named recurring people may die or permanently leave when earned.

Death is not followed automatically by one-for-one replacement.

Vacancy may create:
- workload redistribution;
- understaffing;
- delayed missions;
- reluctant promotions;
- cross-training;
- greater McCluster/Wildcard dependence;
- support-character rise;
- permanent capability loss.

If a replacement eventually arrives:

> **They inherit the vacancy, not the dead person's personality or relationships.**

They should already know some people and be unknown to others. Equal-distance introductions are unrealistic.

Full authority: `story/CAST-LIFECYCLE-ATTRITION-PROMOTION-AUTHORITY.md`.

---

# 12. Ensemble gameplay law

Normal missions launch with named recurring people assigned to the operation, not McCluster by default.

Recurring gameplay creates attachment. Losing someone hurts because the player has worked with them, watched them eat, argue, improve and become useful—not because a dossier says they matter.

PRIM2 is the rarer second-stage intervention. Royce is the opposing elite escalation.

---

# Governing ensemble statement

> **The succession belongs to McCluster. The world belongs to everybody.**

Practical rules:

> **Same building. Same institution. Same people. New pressure.**

> **If you appear, you move. If you die, the building changes.**